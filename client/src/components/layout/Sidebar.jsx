...
const NavBtn = ({
  ...
  return (
    <Tooltip title={collapsed ? label : ""} placement="right" arrow>
      ...
      >
        <Box sx={{ width: collapsed ? COLLAPSED_WIDTH : 44, minWidth: collapsed ? COLLAPSED_WIDTH : 44, display: "flex", alignItems: "center", justifyContent: "center", color: clr, fontSize: 18, flexShrink: 0, pl: indent && !collapsed ? 0.5 : 0 }}>
          {icon}
        </Box>
        {!collapsed ? (
          <Typography sx={{ fontSize: indent ? 12.5 : 13, fontWeight: active || subActive ? 700 : 500, color: txtClr, lineHeight: 1, flex: 1, whiteSpace: "nowrap" }}>
            {label}
          </Typography>
        ) : null}
...
const Sidebar = ({ open = true, onToggle }) => {
  ...
  const canManageBusiness = isAdmin || isOwner;
  ...
  const settingsPaths = ["/company-profile", "/invoice-settings", "/product-defaults", "/user-management", "/backup-data", ...(isOwner ? ["/license-management"] : [])];
  const isSettingsActive = settingsPaths.includes(location.pathname);
  ...
  const settingsSubItems = [
    ...
    ...(isOwner ? [{ label: "License Control", icon: <SettingsIcon sx={{ fontSize: 16 }} />, path: "/license-management" }] : []),
  ];
  ...
  return (
    <Box sx={{ width: w, minWidth: w, height: "100vh", position: "sticky", top: 0, alignSelf: "flex-start", background: S.bg, borderRight: `1px solid ${S.border}`, display: "flex", flexDirection: "column", transition: "width .22s cubic-bezier(.4,0,.2,1), min-width .22s cubic-bezier(.4,0,.2,1)", overflow: "hidden", zIndex: 100 }}>
      <Box sx={{ height: 52, display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "space-between", px: collapsed ? 0.8 : 1.1, borderBottom: `1px solid ${S.border}`, flexShrink: 0 }}>
        {!collapsed ? (
          <Box sx={{ display: "flex", alignItems: "center", gap: 0, height: "100%", pl: 0 }}>
            <Box sx={{ width: 3, height: "100%", background: S.accent, flexShrink: 0 }} />
            <Box sx={{ pl: 2 }}>
              <Typography sx={{ fontSize: 16, fontWeight: 900, color: S.text, lineHeight: 1, fontFamily: "'DM Mono', monospace" }}>
                Renic <Box component="span" sx={{ color: S.accent }}>Bill</Box>
              </Typography>
        ...
        {!collapsed ? <SideToggleBtn collapsed={collapsed} onClick={onToggle} /> : null}
      </Box>

      <Box sx={{ flex: 1, overflowY: "auto", overflowX: "hidden", pb: 1 }}>
        <NavBtn icon={<DashboardIcon sx={{ fontSize: 18 }} />} label="Dashboard" active={location.pathname === "/"} to="/" collapsed={collapsed} />

        <NavBtn icon={<PointOfSaleIcon sx={{ fontSize: 18 }} />} label="Sales" active={isSalesActive && !collapsed} collapsed={collapsed} hasChildren isOpen={salesOpen} onClick={() => setSalesOpen((v) => !v)} />
        {!collapsed ? (
          <Collapse in={salesOpen} timeout={160} unmountOnExit>
            <SubGroup>
              {salesSubItems.map((sub) => (
                <NavBtn key={sub.path} icon={sub.icon} label={sub.label} subActive={location.pathname === sub.path} to={sub.path} indent collapsed={false} />
              ))}
            </SubGroup>
          </Collapse>
        ) : null}
        ...
        {canManageBusiness ? (
          <>
            <NavBtn icon={<LocalShippingIcon sx={{ fontSize: 18 }} />} label="Purchasement" active={isPurchaseActive && !collapsed} collapsed={collapsed} hasChildren isOpen={purchaseOpen} onClick={() => { navigate("/suppliers/products"); setPurchaseOpen((v) => !v); }} />
            {!collapsed ? (
        ...
        {canManageBusiness ? (
          <>
            <NavBtn icon={<PeopleIcon sx={{ fontSize: 18 }} />} label="Customers" active={isCustomerActive && !collapsed} collapsed={collapsed} hasChildren isOpen={customerOpen} onClick={() => { navigate("/customers"); setCustomerOpen((v) => !v); }} />
            {!collapsed ? (
        ...
        {canManageBusiness ? (
          <>
            <NavBtn icon={<AddBusinessIcon sx={{ fontSize: 18 }} />} label="Suppliers" active={isSupplierActive && !collapsed} collapsed={collapsed} hasChildren isOpen={supplierOpen} onClick={() => { navigate("/suppliers"); setSupplierOpen((v) => !v); }} />
            {!collapsed ? (
        ...
        <NavBtn icon={<InventoryIcon sx={{ fontSize: 18 }} />} label="Inventory" active={isProductsActive && !collapsed} collapsed={collapsed} hasChildren isOpen={productsOpen} onClick={() => { navigate("/products"); setProductsOpen((v) => !v); }} />
        {!collapsed ? (
        ...
        {canManageBusiness ? <NavBtn icon={<AssessmentIcon sx={{ fontSize: 18 }} />} label="Reports" active={location.pathname === "/reports"} to="/reports" collapsed={collapsed} /> : null}

        {canManageBusiness ? (
          <>
            <NavBtn icon={<SettingsIcon sx={{ fontSize: 18 }} />} label="Settings" active={isSettingsActive && !collapsed} collapsed={collapsed} hasChildren isOpen={settingsOpen} onClick={() => setSettingsOpen((v) => !v)} />
            {!collapsed ? (
      ...
      <Box sx={{ borderTop: `1px solid ${S.border}`, flexShrink: 0 }}>
        {collapsed ? (
          <Tooltip title={user?.name || "User"} placement="right">
            <Box onClick={(e) => setAnchorEl(e.currentTarget)} sx={{ height: 52, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", "&:hover": { background: S.elevated } }}>
          ...
          <Box onClick={(e) => setAnchorEl(e.currentTarget)} sx={{ display: "flex", alignItems: "center", gap: 1.4, px: 1.6, py: 1.3, cursor: "pointer", borderTop: `2px solid ${S.accent}`, "&:hover": { background: S.elevated } }}>
            <Avatar sx={{ width: 34, height: 34, fontSize: 13, fontWeight: 900, bgcolor: S.accent, color: S.bg, flexShrink: 0, borderRadius: 0 }}>
              {user?.name?.[0]?.toUpperCase() || "U"}
            </Avatar>

            <Box sx={{ flex: 1, overflow: "hidden" }}>
              <Typography noWrap sx={{ fontSize: 13, fontWeight: 700, color: S.text, lineHeight: 1.3 }}>{user?.name || "User"}</Typography>
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.8, mt: 0.3 }}>
                <Box sx={{ fontSize: 9.5, fontWeight: 800, letterSpacing: ".06em", textTransform: "uppercase", px: 0.8, py: "1px", background: isPrivileged ? S.accent : S.border, color: isPrivileged ? S.bg : S.textSub, lineHeight: 1.6 }}>
                  {roleLabel}
                </Box>
                {user?.phone ? <Typography noWrap sx={{ fontSize: 10.5, color: S.textMute }}>{user.phone}</Typography> : null}
              </Box>
            </Box>
        ...
        <Menu
          ...
          PaperProps={{ sx: { borderRadius: 0, border: "1px solid #e2eaf4", boxShadow: "0 8px 32px rgba(0,0,0,.12)", minWidth: 190 } }}
        >
          <Box sx={{ px: 2, py: 1.4, borderBottom: "1px solid #f1f5f9", background: "#fafbfd" }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.6, mt: 0.4 }}>
              <Box sx={{ fontSize: 9.5, fontWeight: 800, letterSpacing: ".06em", textTransform: "uppercase", px: 0.8, py: "1px", background: isPrivileged ? "#fef3c7" : "#f1f5f9", color: isPrivileged ? "#92400e" : "#64748b", lineHeight: 1.6 }}>
                {roleLabel}
              </Box>
              {user?.phone ? <Typography sx={{ fontSize: 11, color: "#94a3b8" }}>{user.phone}</Typography> : null}
            </Box>
          </Box>

          <MenuItem onClick={handleLogout} sx={{ fontSize: 13, fontWeight: 600, color: "#dc2626", gap: 1.2, py: 1.2, borderRadius: 0, "&:hover": { background: "#fef2f2" } }}>
